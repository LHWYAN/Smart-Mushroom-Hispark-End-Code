...
static void handle_cloud_command(const char *payload)
{
    ...
    if (strcmp(cmd_name, "SetLamp") == 0) {
    ...
    } else if (strcmp(cmd_name, "SetBuzzer") == 0) {
        if (extract_json_string_value(payload, "BuzzerStatus", para_val, sizeof(para_val)) == 0) {
            if (strcmp(para_val, "ON") == 0) {
                buzzerState = 1;
                Buzzer_On();
        ...
        printf("[MQTT] Command SetBuzzer, BuzzerStatus=%s, Result=%s\r\n", para_val, result);
    } else {
        return;
    }
...
static void Setup_JSON_Tree_JX(void)
{
    ...
    snprintf_s(A_JSON_Tree, sizeof(A_JSON_Tree), sizeof(A_JSON_Tree) - 1,
               JSON_Tree_Format, tempStr, humiStr, lumiStr, LampSt, CondiSt, VentSt, BuzzerSt, SmokeSt, CO2St);
